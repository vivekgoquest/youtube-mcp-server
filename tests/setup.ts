...
// Load API key from tests/.env.test first
const envTestPath = path.join(__dirname, ".env.test");
if (!process.env.YOUTUBE_API_KEY && fs.existsSync(envTestPath)) {
  const key = fs.readFileSync(envTestPath, "utf8").trim();
  if (key) {
    process.env.YOUTUBE_API_KEY = key;
    console.log("✅ API key loaded from tests/.env.test");
  }
}
...
export const REAL_CLIENT = new YouTubeClient({
  apiKey: process.env.YOUTUBE_API_KEY!,
});

// Helper function to add delays between API calls to respect rate limits
export const apiDelay = (ms: number = 100) =>
...
export const QUOTA_BUDGET_LIMIT = parseInt(
  process.env.MAX_QUOTA_FOR_COMPLIANCE_TESTS || "500",
);

export const trackQuotaUsage = (amount: number) => {
  quotaUsedInTests += amount;
  console.log(
    `Quota used: ${amount} (Total: ${quotaUsedInTests}/${QUOTA_BUDGET_LIMIT})`,
  );
};
...
export const isComplianceTestMode = () => {
  return (
    ...
    process.env.COMPLIANCE_TEST_MODE === "minimal"
  );
};
