...
describe('MCP Diagnostics CLI', () => {
  const CLI_PATH = path.join(__dirname, '../../dist/src/cli/mcp-diagnostics.js');
  const PROJECT_ROOT = path.join(__dirname, '../..');
  
  // Helper function to run CLI commands
  const runCLI = (args: string[], options: { timeout?: number; env?: Record<string, string> } = {}): Promise<{
    stdout: string;
    stderr: string;
    exitCode: number;
  }> => {
  ...
  describe('Health Subcommand', () => {
    test('runs health check with valid API key', async () => {
      ...
      if (!apiKey) {
        console.log('⚠️  Skipping health check test - no YOUTUBE_API_KEY available');
        return;
      }
    ...
    test('supports JSON output format', async () => {
      ...
      if (!apiKey) {
        console.log('⚠️  Skipping JSON health test - no YOUTUBE_API_KEY available');
        return;
      }
  ...
  describe('Discovery Subcommand', () => {
    ...
    test('generates JSON report file', async () => {
      ...
      if (reportExists) {
        ...
        expect(report).toHaveProperty('developmentMode');
        expect(report).toHaveProperty('productionMode');
      }
    });

    test('supports verbose output', async () => {
      const result = await runCLI(['discovery', '--verbose'], { timeout: 45000 });
    ...
    test('handles development and production modes', async () => {
      const result = await runCLI(['discovery'], { timeout: 45000 });
