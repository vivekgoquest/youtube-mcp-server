  ...
  inputSchema: {
    ...
    properties: {
      ...
      maxResults: {
        type: "integer",
        description:
          "Maximum number of analyzed keywords to return (default: 50)",
        minimum: 1,
        maximum: 100,
        default: 50,
      },
...
export default class AnalyzeKeywordsTool
  implements ToolRunner<AnalyzeKeywordsOptions, KeywordAnalysisResult>
{
  ...
  async run(
    ...
    try {
      ...
      for (const keyword of options.keywords) {
        ...
        const relevance = this.calculateKeywordRelevance(
          keyword,
      ...
      const competitionAnalysis = options.includeCompetitionAnalysis
        ? await this.performCompetitionAnalysis(keywordData)
        : undefined;
  ...
  private extractTheme(keywords: KeywordData[]): string {
    ...
    return (
      Object.entries(wordCount).sort(([, a], [, b]) => b - a)[0]?.[0] ||
      "General"
    );
  }
  ...
  private generateKeywordInsights(keywords: KeywordData[]): {
    ...
    return {
      ...
      emergingKeywords: keywords
        .filter(
          (k) => (k.searchVolume || 0) > 1000 && (k.competition || 0) < 50,
        )
        .slice(0, 10),
      competitiveKeywords: keywords
        .filter((k) => (k.competition || 0) > 70)
        .slice(0, 10),
  ...
  private generateRecommendations(
    ...
    if (insights.opportunityKeywords.length > 0) {
      recommendations.push(
        `Focus on ${insights.opportunityKeywords.length} low-competition keywords with good search volume`,
      );
    }

    if (insights.emergingKeywords.length > 0) {
      recommendations.push(
        `Consider targeting ${insights.emergingKeywords.length} emerging keywords for future content`,
      );
    }

    recommendations.push(
      "Analyze competitor content for the top competitive keywords",
    );
    recommendations.push(
      "Create content clusters around the identified keyword themes",
    );
