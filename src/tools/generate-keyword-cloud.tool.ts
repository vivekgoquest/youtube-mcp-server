...
export default class GenerateKeywordCloudTool implements ToolRunner<GenerateKeywordCloudOptions, KeywordCloudData> {
  constructor(private client: YouTubeClient) {}

  async run(options: GenerateKeywordCloudOptions): Promise<ToolResponse<KeywordCloudData>> {
    ...
    try {
      ...
      const processedKeywords = this.processKeywordsForCloud(options.keywords, options);
  ...
  private processKeywordsForCloud(keywords: string[], options: GenerateKeywordCloudOptions): Array<{
    text: string;
    weight: number;
    frequency: number;
    ...
    return Object.entries(keywordFreq).map(([keyword, frequency]) => {
      ...
      const opportunityScore = this.calculateOpportunityScore(keyword, frequency);
  ...
  private calculateOpportunityScore(keyword: string, frequency: number): number {
  ...
  private generateClusters(keywords: Array<{
    text: string;
    weight: number;
    frequency: number;
    opportunityScore: number;
      ...
      if (relatedKeywords.length > 1) {
        const cluster: KeywordCluster = {
          ...
          relatedKeywords: relatedKeywords.map(k => ({
            ...
            sources: k.sources.filter(s => ['title', 'description', 'tags', 'comments'].includes(s)) as ('title' | 'description' | 'tags' | 'comments')[],
          ...
          opportunityScore: relatedKeywords.reduce((sum, k) => sum + k.opportunityScore, 0) / relatedKeywords.length,
  ...
  private areKeywordsSimilar(keyword1: string, keyword2: string): boolean {
    ...
    const words1 = keyword1.toLowerCase().split(' ').filter(w => w.length > 2);
    const words2 = keyword2.toLowerCase().split(' ').filter(w => w.length > 2);
  ...
  private extractTheme(keywords: string[]): string {
    ...
    const words = keywords.flatMap(k => k.split(' ')).filter(w => w.length > 2);
