...
import { YouTubeClientConfig, ToolResponse } from "./types.js";
...
export class YouTubeMCPServer {
  ...
  private formatToolResult(toolName: string, data: any): string {
    ...
    switch (toolName) {
      case "unified_search":
        if (data.items) {
          const searchType = data.items[0]?.id?.videoId
            ? "videos"
  ...
  private formatChannelVideosAnalysis(data: any): string {
    ...
    const avgLikes = Math.round(
      allVideos.reduce((sum: number, v: any) => sum + v.likes, 0) /
        allVideos.length,
    );
    const avgComments = Math.round(
      allVideos.reduce((sum: number, v: any) => sum + v.comments, 0) /
        allVideos.length,
    );

    // Calculate engagement rate (likes + comments / views)
    const avgEngagement =
      allVideos.reduce((sum: number, v: any) => {
        const engagement =
    ...
    // Duration analysis
    const durationCounts = allVideos.reduce(
    ...
    if (recentVideos.length > 0 && olderVideos.length > 0) {
      const recentAvgViews = Math.round(
        recentVideos.reduce((sum: number, v: any) => sum + v.viewsPerMonth, 0) /
          recentVideos.length,
      );
      const olderAvgViews = Math.round(
        olderVideos.reduce((sum: number, v: any) => sum + v.viewsPerMonth, 0) /
          olderVideos.length,
      );
